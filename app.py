...
from langgraph_backend_database import chat_bot, retrieve_all_threads, delete_thread
...
# make sure current thread is in the list
add_thread(st.session_state['thread_id'])

# rebuild titles from saved messages
...
# load visible messages for the active thread after refresh / reload
...
st.sidebar.markdown(
    ...
    "A multi-thread conversational AI built with LangGraph, SQLite and Streamlit Frontend by Junaeid Shoaib"
    "</div>",
...
if user_input:
        ...
        # if this chat still has default title, rename it using first user message
